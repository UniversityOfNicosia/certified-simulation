...
- replays every certified identity, probability law, and
  counterexample in **exact rational arithmetic** — no floating
  point, no tolerances, no random seeds in any certified quantity;
...
ones by hash; under a different TeX distribution, byte-identity is
not guaranteed (TeX embeds toolchain details), and the check becomes:
the build completes with no unresolved references or citations, and
the rendered content matches.
...
Levels 0–2 establish that the released record is internally exact:
the bytes, the computations, and the documents agree with each other.
They do not by themselves prove the theorems — that is Level 3, by
design in your hands — and every certificate checks exactly the
